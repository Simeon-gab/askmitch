...
// /test — the full registration flow, redesigned (visual lab).
// Copy is verbatim from docs/DESIGN.md; only presentation changes.
...
// The lab NEVER calls POST /api/register — the voucher screen shows a
// hard-coded sample code so no junk leads ever reach the database.
...
// Sample only — real codes are generated server-side per lib/voucher.ts
// (MITCH- + 5 chars). This one is the docs/ARCHITECTURE.md example.
const SAMPLE_CODE = "MITCH-K3XT9";
        ...
        <main className={s.stage} data-dir={dir}>
          ...
          <section className={scr(7)} aria-hidden={step !== 7}>
            ...
            <div className={voucherPop ? `${s.voucher} ${s.popped}` : s.voucher}>
              ...
              <div className={s.code}>{SAMPLE_CODE}</div>
              <button
                suppressHydrationWarning
                type="button"
                className={s.copybtn}
